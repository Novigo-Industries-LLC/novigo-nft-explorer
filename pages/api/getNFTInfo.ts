...
export default async function handler(
  ...
  if(nftAddress) {
    const nftData = await (await Moralis.SolApi.nft.getNFTMetadata({ address: nftAddress as SolAddressish })).toJSON();
    const { data: nftMetadata } = await axios.get(nftData.metaplex.metadataUri);
    return res.status(200).json({ nftData: nftData, metadata: nftMetadata });
  }
